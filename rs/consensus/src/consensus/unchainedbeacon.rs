...
use ic_types::{
    consensus::certification::Certification, unchainedbeacon::UnchainedBeaconDelivery,
...
pub trait UnchainedBeaconMessageRouting {
...
impl UnchainedBeaconMessageRouting for UnchainedBeaconClient {
    fn deliver_batch(&self, batch: Vec<UnchainedBeaconDelivery>) {
        for entry in batch {
            ...
            self.certification_pending.queue_certification(
                consensus_height,
                Height::from(0),
                CryptoHashOfPartialState::from(CryptoHash(consensus_height)),
            );
        }
    }
...
pub struct UnchainedBeacon {
    pub ub_message_routing: Arc<dyn UnchainedBeaconMessageRouting>,
...
impl UnchainedBeacon {
    ...
    fn new(
        ub_message_routing: Arc<dyn UnchainedBeaconMessageRouting>,
