...
BUILD_BINARIES_CMD=$(
    ...
    bazel cquery --config=local --output=files //publish/binaries | xargs -I {} cp -f {} "$BINARIES_DIR"
END
)
...
BUILD_CANISTERS_CMD=$(
    ...
    bazel cquery --config=local --output=files //publish/canisters | xargs -I {} cp -f {} "$CANISTERS_DIR"
END
)
...
BUILD_IMAGES_CMD=$(
    ...
    bazel cquery --config=local --output=files //ic-os/guestos/envs/${IMG_TYPE} | xargs -I {} cp -f {} "${DISK_DIR}/guestos"

    # build hostos images
    #mkdir -p "${DISK_DIR}/hostos"
    #$BAZEL_CMD //ic-os/hostos/envs/${IMG_TYPE}
    #bazel cquery --config=local --output=files //ic-os/hostos/envs/${IMG_TYPE} | xargs -I {} cp -f {} "${DISK_DIR}/hostos"

    # build setupos images
    #mkdir -p "${DISK_DIR}/setupos"
    #$BAZEL_CMD //ic-os/setupos/envs/${IMG_TYPE}
    #bazel cquery --config=local --output=files //ic-os/setupos/envs/${IMG_TYPE} | xargs -I {} cp -f {} "${DISK_DIR}/setupos"
END
)
BUILD_CMD=""
